...
export class MergifyReporter implements Reporter {
  ...
  onInit(vitest: Vitest): void {
    ...
    // Fetch quarantine list and configure runner
    if (this.tracing && token && repoName) {
  ...
  private _configureRunner(vitest: Vitest): void {
    ...
    vitest.provide('mergify:quarantine' as never, [...this.quarantineList] as never);

    // Auto-configure the custom runner if not already set
    if (!vitest.config.runner) {
      const dir = dirname(fileURLToPath(import.meta.url));
      vitest.config.runner = resolve(dir, 'runner.js');
    }
  }
